...
def yolo_layer(name, inputs, anchors, num_classes, image_height, image_width):
    ...
    with tf.variable_scope(name):
        ...
        # [ ? x 13 x 13 x num_anchors x (6 + num_classes)]
        inputs = tf.reshape(inputs, [-1,inputs_shape[1], inputs_shape[2], num_anchors, num_classes + 6])
        delta_x, delta_y, delta_w, delta_h, delta_theta, obj_conf, class_conf = tf.split(inputs, [1, 1, 1, 1, 1, 1, num_classes], axis=-1)
        ...
        theta = tf.nn.sigmoid(delta_theta)
        obj_conf = tf.nn.sigmoid(obj_conf)
        class_conf = tf.nn.sigmoid(class_conf)

        inputs = tf.concat([box_x, box_y, box_w, box_h, theta, obj_conf, class_conf], axis=-1)
