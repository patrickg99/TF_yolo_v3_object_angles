...
def yolo_v3(inputs, num_classes, anchors, h=416, w=416, training=False):
    ...
    large_object_box_detections : tensor
        ...
        Tensor of shape:
        [batch_size, large_yolo_layer_grid_h, large_yolo_layer_grid_w, num_anchors_per_layer, 6 + num_classes]
    ...
    medium_object_box_detections : tensor
        ...
        Tensor of shape:
        [batch_size, med_yolo_layer_grid_h, med_yolo_layer_grid_w, num_anchors_per_layer, 6 + num_classes]
    
    small_object_box_detections : tensor
        Contains the detectors responsible for predicting the bounding boxes of small objects. 
        Tensor of shape:
        [batch_size, small_yolo_layer_grid_h, small_yolo_layer_grid_w, num_anchors_per_layer, 6 + num_classes]
    ...
    large_object_raw_detections, _ = convolutional_layer(name='conv_58', inputs=inputs, filters=3*(num_classes+6), kernel_size=1, downsample=False, batch_norm=False, activation='LINEAR')
    ...
    medium_object_raw_detections, _ = convolutional_layer(name='conv_66', inputs=inputs, filters=3*(num_classes+6), kernel_size=1, downsample=False, batch_norm=False, activation='LINEAR')
    ...
    small_object_raw_detections, _ = convolutional_layer(name='conv_74', inputs=inputs, filters=3*(num_classes+6), kernel_size=1, downsample=False, batch_norm=False, activation='LINEAR')
    ...
    if training == False:
        ...
        with tf.variable_scope('reshape_yolo_0'):
            large_object_box_detections = tf.reshape(large_object_box_detections, [-1, large_object_box_detections.shape[1]*large_object_box_detections.shape[2]*large_object_box_detections.shape[3], num_classes + 6])
        with tf.variable_scope('reshape_yolo_1'):
            medium_object_box_detections = tf.reshape(medium_object_box_detections, [-1, medium_object_box_detections.shape[1]*medium_object_box_detections.shape[2]*medium_object_box_detections.shape[3], num_classes + 6])
        with tf.variable_scope('reshape_yolo_2'):
            small_object_box_detections = tf.reshape(small_object_box_detections, [-1, small_object_box_detections.shape[1]*small_object_box_detections.shape[2]*small_object_box_detections.shape[3], num_classes + 6])
